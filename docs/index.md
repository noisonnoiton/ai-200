...
이 사이트는 방대한 전체 문서를 대체하지 않습니다. **하루 세미나에서 전체 지형과 서비스 선택 기준을 잡는 것**을 목표로 합니다.
...
2. [하루 세미나](seminar/day-plan.md)를 따라 네 domain을 훑습니다.
3. [End-to-end architecture](architecture/end-to-end.md)에서 서비스가 연결되는 이유를 봅니다.
4. 각 domain의 선택 기준과 구현 checklist를 복습합니다.

!!! note "범위"
    이 자료는 overview와 세미나용 요약입니다. 실제 시험 준비에는 Azure SDK를 사용한 Python 구현, container 배포, KQL 작성 실습이 추가로 필요합니다.
